    ...
    return (
      <div className="w-full py-16 px-6 mt-22 sm:px-10 bg-gradient-to-b from-active to-white rounded-3xl">
        ...
        <div className="flex flex-col items-center justify-center py-20 px-6 text-center bg-white rounded-2xl  border-t border-gray-100">
          ...
          <h3 className="text-lg font-[450] text-gray-700 mb-2">No news found</h3>
        ...
        {news.slice(0, visibleCount).map((item) => (
          ...
          >
            <div className="relative rounded-xl p-3 sm:p-5 bg-white hover:bg-secondary flex flex-col justify-between transition-shadow duration-300 hover:shadow-lg group">
              {/* Category */}
              {item.category && (
                <p className="text-xs uppercase mb-2 text-primary font-[450]">
                  {item.category.name}
                </p>
              )}
              ...
              {/* Title */}
              {item.title && (
                <h2 className="text-lg sm:text-xl md:text-2xl font-[450] mb-4 text-gray-800">
                  {item.title}
                </h2>
              )}
      ...
      {visibleCount < news.length && (
        <div className="flex justify-center mt-10">
          <button
            ...
            className="px-6 py-3 bg-primary hover:bg-tertiary text-light font-[450] rounded-full transition-colors duration-300"
          >
            Load More
          </button>
        </div>
